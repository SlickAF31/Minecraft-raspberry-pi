...
sleep(15) #timpul de standby din momentul activarii camerei pana la inchiderea ei
camera.capture('selfie.jpg')  #salvarea pozei
camera.close()



#maparea culorilor si a pozei salvate
...
#maparea rgb-block-uri minecraft 

mc = Minecraft.create()
x, y, z = mc.player.getPos()


#randarea si transformarea din .jpg in tablou in minecraft
		...
		for k, map_column in enumerate(map_lab):
			for l, map_pixel in enumerate(map_column):
				...
				if delta < distance:
					distance = delta
					block = colours[(k,l)]
		mc.setBlock(x-j, y-i+60, z+5, block[0], block[1]) #pozitionarea pozei la o distanta fata de coordonatele playerului